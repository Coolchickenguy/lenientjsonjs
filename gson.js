...
var _lenientCommentsGlobal = /(?:(?:(?:\/\/)|#).*$)|(?:\/\*[\s\S]*?\*\/)/gm;
...
function _parseString() {
  ...
  if (boundary) stringContainer = new RegExp("[^" + boundary + "\\\\]*");
  else stringContainer = new RegExp("\\w*");

  while (true) {
    var grp = eatRegex(stringContainer);

    if (grp !== false) str += grp;

    if (boundary === null || eatChar(boundary)) break;
    else if (eatRegex(_stringEscape)) {
      var ctrl = eatRegex(
        !_strict ? _lenientControlCharacter : _controlCharacter
      );
    ...
    } else _error('expected " or \\, got ' + peekChar());
  }
...
function _parseObject() {
  ...
  if (eatRegex(_startObject) === false) _error("expected {, got " + peekChar());
  ...
  while (true) {
    ...
    if (eatRegex(_comma)) continue;
    // "Name/value pairs separated by ; instead of ,."
    else if (!_strict && eatRegex(_lenientSemicolon)) continue;
    else if (!eatRegex(_endObject)) _error("expected }, got " + peekChar());
...
function _parseArray() {
  ...
  if (eatRegex(_startArray) === false) _error("expected [, got " + peekChar());
  ...
  while (true) {
    ...
    else if (!eatRegex(_endArray)) _error("expected ], got " + peekChar());
...
function _parseNumber() {
  ...
  if ((str = eatRegex(_number)) === false) {
    ...
    if (!_strict) {
      if ((str = eatRegex(_lenientNaNOrInfinite)) !== false) return Number(str);
    }
...
function _parseNull() {
  if (eatRegex(_null) === false) _error("expected null, got " + peekChar());
...
function _parseAny() {
  ...
  if (peekRegex(_startObject)) return _parseObject();
  else if (peekRegex(_startArray)) return _parseArray();
  else if (peekRegex(_number) || (!_strict && peekRegex(_lenientNaNOrInfinite)))
    return _parseNumber();
  else if (peekRegex(_boolean)) return _parseBoolean();
  else if (peekRegex(_null)) return _parseNull();
  else if (
    peekRegex(_stringBoundary) ||
    (!_strict &&
      (peekRegex(_lenientStringBoundary) || peekRegex(_lenientStringStart)))
  )
    return _parseString();
...
function parse(json, strict) {
  ...
  if (!_strict) eatRegex(_lenientNonExecutePrefix);

  // "Top-level values of any type. With strict parsing, the top-level value must be an object or an array."
  if (!_strict) return _parseAny();
...
function _opt(name, def) {
  if (!_options || _options[name] === undefined) return def;
